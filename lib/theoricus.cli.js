...
(function() {
  ...
  __t('theoricus.commands', exports).Compiler = (function() {
    var ArrayUtil, FsUtil, Toaster, config, fs, jade, path, stylus;
    ...
    function Compiler(the, options, after_compile) {
      var config_contents,
        _this = this;
      this.the = the;
      config_contents = config.replace(/~THE/g, this.the.root);
      config_contents = config_contents.replace("~APP", "" + this.the.pwd + "/app");
      this.toaster = new Toaster(this.the.pwd, config_contents, {
        w: 1,
        d: 1
      }, function() {
        _this.compile(function() {
          if (typeof after_compile === "function") {
            after_compile();
          }
          return after_compile = null;
        });
        return false;
      });
      FsUtil.watch_folder(this.the.pwd, /.jade$/, function(info) {
        if (info.action !== "watching") {
          return _this.compile();
        }
      });
      FsUtil.watch_folder(this.the.pwd, /.styl$/, function(info) {
        if (info.action !== "watching") {
          return _this.compile();
        }
      });
    ...
    Compiler.prototype.compile_jade = function(after_compile) {
      var _this = this;
      return FsUtil.find(this.the.pwd, "*.jade", function(files) {
        var alias, buffer, compiled, file, folder, included, output, source, _i, _len;
        ...
        output = _this.to_single_line(output);
        return after_compile("// TEMPLATES\n" + output);
      });
    };

    Compiler.prototype.compile_stylus = function(after_compile) {
      var _this = this;
      return FsUtil.find(this.the.pwd, "*.styl", function(files) {
        var buffer, compiled, file, paths, pending, source, _i, _len, _results;
        buffer = [];
        pending = 0;
        _results = [];
        for (_i = 0, _len = files.length; _i < _len; _i++) {
          ...
          pending++;
          source = fs.readFileSync(file, "utf-8");
          paths = ["" + _this.the.pwd + "/app/views/_mixins/stylus"];
          _results.push(compiled = stylus(source).set('filename', file).set('paths', paths).render(function(err, css) {
            if (err != null) {
              throw err;
            }
            buffer.push(css);
    ...
    Compiler.prototype.compile = function(after_compile) {
      var conf,
        _this = this;
      conf = this._get_config();
      return this.compile_jade(function(templates) {
        var footer, header;
        header = "" + templates + "\n\n" + conf.config + "\n\n" + conf.routes + "\n\n" + conf.root + "\n";
        footer = "new theoricus.Theoricus";
        return _this.toaster.build(header, footer, function() {
          return _this.compile_stylus(function(css) {
            var target;
            target = "" + _this.the.pwd + "/public/app.css";
            fs.writeFileSync(target, css);
            return typeof after_compile === "function" ? after_compile() : void 0;
          });
        });
      });
    };
  ...
  __t('theoricus.commands', exports).Server = (function() {
    ...
    function Server(the, options) {
      ...
      this.compiler = new theoricus.commands.Compiler(this.the, options, function() {
